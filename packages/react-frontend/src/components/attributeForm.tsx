...
function AttributeForm(props) {
    ...
    return (
        <div className="attributeForm">
            <label>
                Attribute Name:
                <input onChange={(event) => handleNameChange(event.target.value)} />
            </label>
            <label>
                Unit:
                <select onChange={(event) => handleUnitChange(event.target.value)}>
                    {listAttributeEnum}
                </select>
            </label>
            <input type="button" value="Add Attribute" onClick={handleAddAttribute} />
        </div>
    )
}
